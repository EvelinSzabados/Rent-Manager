  ...
  return (
    <React.Fragment>
      ...
      {notification.length === 0 ? (
        <h2 className="notification_header"> No Notifications</h2>
      ) : (
      ...
      {notification.map((notification) =>
        notification.rentedProductsDetails === null ? (
          ...
          <div
            ...
            key={"notif_container_" + notification.id}
            id={"notif_container_" + notification.id}
          >
            <table
              className="notification_table"
              key={"table_" + notification.id}
            >
              <caption key={"caption_" + notification.id}>
                Customer name:
                {" " +
              ...
              {notification.rentedProductsDetails === null
                ? ""
                : notification.rentedProductsDetails.map((rentedProduct) => (
                    <tbody key={"tbody_" + rentedProduct.id}>
                      <tr key={"tr_" + rentedProduct.id}>
                        <td
                          className="notif_id"
                          key={"td_id" + rentedProduct.id}
                        >
                          {rentedProduct.id}
                        </td>
                        <td key={"td_name" + rentedProduct.id}>
                          {rentedProduct.name}
                        </td>
                        <td key={"td_date" + rentedProduct.id}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={(e) => {
                              const modifyUrl = `http://localhost:8762/product/product/setStatus/${rentedProduct.id}`;
