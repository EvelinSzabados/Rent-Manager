...
export const addRentValidation = (selectProduct, selectCustomer) => {
    ...
    if (notEmpty([selectProduct, selectCustomer, startDate.value, endDate.value])) {
        if (isBigger(endDate.value, startDate.value)) {
            ...
            const rent = { "customer_id": selectCustomer.id, "cost": cost, "start_date": startDate.value, "enddate": endDate.value, "rentedProducts": rentedProducts }
            addRent(rent)
